...
- **Minimal IR commands** — the card compares against the device's current state and only sends what differs, so the AC doesn't beep for redundant mode/fan commands. Powering on when the unit already remembers the right mode and fan speed is a single command (one beep). The timer is a cloud-side schedule, not an IR command, so it never beeps.
